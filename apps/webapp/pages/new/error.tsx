...
const NewErrorPage = memo(function NewErrorPage() {
  const router = useRouter()
  const { query } = router
  const name = convertQueryParamToString(query.name)
  const alreadyExists = convertQueryParamToString(query.alreadyExists)
  ...
  const span = <span className="text-purple-500">go/{name}</span>
      ...
      <main className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-[#2e026d] to-[#15162c]">
        <div className="container flex w-full max-w-2xl flex-col justify-center gap-2 rounded bg-white px-6 py-7">
          <h1 className="text-2xl font-medium ">
            {alreadyExists ? <>{span} already exists</> : <>Error creating {span}</>}
          </h1>
